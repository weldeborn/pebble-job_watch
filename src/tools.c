...
void TOOL_voDrawTimeline(GContext *ctx, int16_t minutes , char project[20])
{
  int8_t const X_SIZE = 110;
  int8_t const Y_SIZE = 20;
  int8_t const X_OFFSET = 17;
  int8_t const Y_OFFSET = 10;
  int8_t const X_ONE_HOUR = X_SIZE/9;
  ...
  graphics_draw_line(ctx, GPoint(X_OFFSET, Y_OFFSET+(Y_SIZE/2)-1), GPoint(X_OFFSET+X_SIZE,Y_OFFSET+(Y_SIZE/2)-1));
  graphics_draw_line(ctx, GPoint(X_OFFSET, Y_OFFSET+(Y_SIZE/2)+1), GPoint(X_OFFSET+X_SIZE,Y_OFFSET+(Y_SIZE/2)+1));
  ...
  /* Draw progress*/
  graphics_draw_line(ctx, GPoint(X_OFFSET, Y_OFFSET+(Y_SIZE/2)), GPoint(X_OFFSET + arrow_pos,Y_OFFSET+(Y_SIZE/2)));

  /*Show active project*/
  graphics_draw_text(ctx, project, fonts_get_system_font(FONT_KEY_GOTHIC_14),
                          GRect(X_OFFSET, Y_OFFSET + Y_SIZE, X_SIZE, 30),
